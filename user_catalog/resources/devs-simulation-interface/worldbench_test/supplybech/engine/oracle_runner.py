...
# Ensure engine/ directory is on sys.path so kpi_utils can be imported
...
class LLMPolicy(Policy):
    ...
    def _build_order_quantity_dict(self, fg_qty, routing_dict):
        ...
        real_rms = [rm for rm in raw_materials if rm > 0]
        dummy_rms = [rm for rm in raw_materials if rm < 0]
        if not real_rms:
            real_rms = [target_prod]

        for rm_idx in real_rms:
        ...
        for rm_idx in dummy_rms:
            if None not in order_quantity_dict:
                order_quantity_dict[None] = {}
            order_quantity_dict[None][rm_idx] = 0.0
            allowed_upstreams.add('EXTERNAL')

        unknown_upstreams = [key for key in routing_dict.keys() if key not in allowed_upstreams]
        if unknown_upstreams:
            filtered_routing = {k: v for k, v in routing_dict.items() if k in allowed_upstreams}
        else:
            filtered_routing = routing_dict

        for upstream_semantic in filtered_routing.keys():
            qty = self._extract_routing_quantity(filtered_routing, upstream_semantic)
            if qty == 0.0:
                continue
            ...
            if len(supplier_rms) > 1:
                if target_prod in supplier_rms:
                    order_quantity_dict[supplier_idx][target_prod] = qty
                else:
                    raise ValueError(
        ...
        for group_name, config in topo["node_groups"].items():
            for semantic_name in group_to_instances[group_name]:
                ...
                for pid in product_ids:
                    prod = SupplyChainProduct(index=pid)
                    product_objects[pid] = prod
                    node.add_product(prod)
                node.initial_inventory_level = {pid: inv_dict[pid] for pid in product_ids}

                if "bill_of_materials" in config:
                    for fg_key, rm_dict in config["bill_of_materials"].items():
                        fg_pid = int(fg_key)
                        if fg_pid in product_objects:
                            for rm_key, qty in rm_dict.items():
                                rm_pid = int(rm_key)
                                product_objects[fg_pid].set_bill_of_materials(rm_pid, float(qty))
                ...
                if "bill_of_materials" in config:
                    fg_pids = [int(k) for k in config["bill_of_materials"].keys()]
                    node.supply_type = {pid: 'U' for pid in fg_pids if pid in product_ids}
