...
UTILS_DIR = os.path.join(os.path.dirname(__file__), "devs_construct_tree_cr_slim", "materials", "devs_project", "devs_utils")
...
class DEVSExecute(Tool):
    ...
    inputs = {
        "file_or_project_path": {
            "type": "string", 
            "description": "Path to the DEVS model file (e.g., 'devs_models/model.py') or a project directory relative to the working directory."
        },
        "main_file": {
            "type": "string", 
            "description": "If input is a directory, specify the entry point file name (default: main.py). Ignored if input is a file.", 
            "nullable": True
        },
        "timeout": {
            "type": "integer", 
        ...
        "stdout_file": {
            "type": "string", 
            "description": "Path (relative to working_dir) to save the raw standard output (STDOUT).", 
            "nullable": True
        },
        "stderr_file": {
            "type": "string",
            "description": "Path (relative to working_dir) to save the raw standard error (STDERR). Useful for debugging.",
            "nullable": True
        },
        "allowed_libraries": {
            "type": "string", 
    ...
    def forward(self, file_or_project_path: str, timeout: int = 30, 
        ...
        try:
            target_path = (self.working_dir_path / file_or_project_path).resolve()
            
            # 容错处理：如果 Agent 忘记了 devs_models/ 前缀，或者是创建工具默认放到了子文件夹
            if not target_path.exists():
                potential_path = (self.working_dir_path / "devs_models" / file_or_project_path).resolve()
                if potential_path.exists():
                    target_path = potential_path
                else:
                    # 如果都找不到，返回包含工作目录路径的详细错误，帮助 Agent 自检
                    return f"STATUS: FAILED\nReason: File or directory '{file_or_project_path}' not found in working directory '{self.working_dir_path}'."

            # 安全检查：防止路径穿越
            if not str(target_path).startswith(str(self.working_dir_path)):
                return f"STATUS: FAILED\nReason: Access denied. Path '{file_or_project_path}' is outside the working directory."
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                if target_path.is_file():
                    ...
                    if not (temp_dir_path / PROJECT_FOLDER_NAME / target_main_file).exists():
                         return f"STATUS: FAILED\nReason: Main file '{main_file}' not found in project directory '{file_or_project_path}'."
                else:
            ...
            response += f"TARGET: {target_path.name}\n"
...
""".format(allowed_libs=allowed_libs, module_name=module_name)
    ...
    def _extract_key_results(self, stdout: str, stderr: str, success: bool, execution_time: float) -> List[str]:
        ...
        Format example: 
        {"type": "RESULT", "level": "INFO", ..., "event": "KPI Update", "data": {...}}
        """
        results = []
        ...
        for line in lines:
            ...
            try:
                ...
                if msg_type == "RESULT":
                    # 格式化输出：[Event Name] {data_content}
                    # 将 data 字典转为紧凑的字符串显示
                    data_str = json.dumps(data, ensure_ascii=False)
                    results.append(f"[{event}] {data_str}")
                
                elif msg_type == "ERROR":
                    results.append(f"[LOG ERROR] {event}: {json.dumps(data, ensure_ascii=False)}")
