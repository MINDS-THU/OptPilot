...
from mtl import parse as mtl_parse
...
class RuleTracker:
    ...
    def snapshot_run(self, run_meta: Optional[dict] = None):
        ...
        if self.errors:
            for e in self.errors: e['run_index'] = run_stats['run_index']
            self.archived_errors.extend(self.errors)
        if self.warnings:
            for w in self.warnings: w['run_index'] = run_stats['run_index']
    ...
    def to_dict(self):
        ...
        if self.errors: display_errors.extend(self.errors)
        display_warnings = self.archived_warnings if self.archived_warnings else []
        if self.warnings: display_warnings.extend(self.warnings)
        
        if self.is_global_kpi:
            current_stats = {'total_cases': self.total_cases, 'correct_cases': self.correct_cases, 'error_count': len(self.errors)}
            detailed_history = []
        else:
            ...
            current_stats = {'aggregate_total': total_all, 'aggregate_correct': correct_all, 'aggregate_errors': errors_all, 'runs_count': len(self.stats_history)}
            detailed_history = self.stats_history

        result = {
            'rule_id': self.rule_id, 'name': self.name, 'type': self.rule_type.value, 'description': self.description,
            'score': round(self.score, 4), 'weighted_score': round(self.score * self.weight, 4),
            'stats': current_stats, 'run_details': copy.deepcopy(detailed_history),
        }
        if self.is_global_kpi:
...
class BaseValidator(ABC):
    ...
    @abstractmethod
    def define_rules(self): pass
    
    @abstractmethod
    def validate_logic(self): pass
    
    def validate_kpis(self, batch_stats: List[Dict]): pass
    ...
    def register_rule(self, rule_id: str, name: str, rule_type: RuleType, description: str = "", **kwargs):
        ...
        self.rules[rule_id] = RuleTracker(rule_id=rule_id, name=name, rule_type=rule_type, description=description, is_global_kpi=is_kpi, **kwargs)
    
    def validate_log_entry_hook(self, entry: Dict, line_num: int) -> bool: return True
    ...
    def _load_sidecar_extra(self, log_path: str) -> Dict:
        try:
            ...
            if os.path.exists(extra_path):
                with open(extra_path, 'r', encoding='utf-8') as f: return json.load(f)
        except Exception: pass
        return {}
    ...
    def run(self):
        if isinstance(self.log_path, list):
            file_patterns = []
            for p in self.log_path: file_patterns.extend(glob.glob(p))
        else:
            file_patterns = glob.glob(self.log_path) if ('*' in self.log_path or '?' in self.log_path) else [self.log_path]
        
        if not file_patterns: return {"success": False, "error": f"未找到日志文件: {self.log_path}"}
        ...
        for file_path in file_patterns:
            ...
            for rule in self.rules.values():
                if not rule.is_global_kpi: rule.snapshot_run(run_meta=self.current_meta)
        ...
        for r in self.rules.values(): r.calculate_final_score()
    ...
    def _get_result(self):
        ...
        return {
            "success": True, "run_count": len(self.batch_stats), "total_score": round(total_score, 4),
            "type_averages": type_averges, "rule_scores": {k: round(r.score, 4) for k, r in self.rules.items()},
    ...
    def _build_mtl_signal(self, entries: List[Dict], predicate: Callable[[Dict], bool], max_time: float) -> List[tuple]:
        """
    ...
    def verify_mtl_global(self, rule_id: str, formula_str: str, predicates: Dict[str, Callable[[Dict], bool]]):
        """
        [通用] 验证全局 MTL 属性。
    ...
    def verify_mtl_parametric(self, rule_id: str, formula_str: str, 
