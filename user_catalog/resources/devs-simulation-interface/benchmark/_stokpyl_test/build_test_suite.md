...
"cases": [
      {
        ...
        "sim_stdin": "20.5 0.0\n15.0 1.2\n",
        "checker_config": {},
        "seed_mode": "incremental",  // 可选。随机场景：每次 run 使用不同 seed
        "seed_start": 1024,          // 随机种子起始值
        "seed_arg": "--seed"          // CLI 中 seed 参数名
      }
    ]
  }
]
...
当 `test_cases` 中配置了 `seed_mode` / `seed_start` / `seed_arg` 字段时，`build_test_suite.py` 会：

1. 从 `cli_args_schema` 中检测是否需要 seed 策略（若含 `seed` 参数则默认 `incremental`）。
2. 从 `sim_args` 中剥离 seed 参数，将其写入 case 级别的 `seed_mode/seed_start/seed_arg`。
3. 生成的 `config.json` 中包含这些字段，供 `eval_runner.py` 在每次 run 时按递增模式注入不同 seed。

确定性场景无需这些字段；`eval_runner.py` 的 `resolve_sim_args_for_run()` 在 `seed_mode` 未设置时不做种子干预。
