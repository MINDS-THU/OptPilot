...
**随机场景额外要求**：若场景含有随机逻辑（随机需求、噪声、采样），必须在 `cli_args_schema` 中额外声明 `seed` 参数：
...
该参数由 `oracle_runner.py` 自动注入到 `DYNAMIC_ARGS` 和 `stockpyl` 仿真引擎中，确保每次运行可复现。确定性场景无需此参数。
...
### 6.0 Docstring 质量要求（强制）

`build_description.py` 会通过反射读取 hook 函数的 docstring 并将其直接编入 `description.yaml`。**docstring 是 LLM 理解物理行为的唯一文字来源**，必须满足以下标准：

1. **必须覆盖的信息**（每条 hook）：
   - **作用范围**：作用于哪个产品（如 "only product 1"）和/或哪个节点组。
   - **触发条件**：什么情况下该逻辑生效（如 "when inventory position is at most 50"）。
   - **精确计算**：用具体数字和公式描述（如 "return 1.4 * positive_inventory"），不要用含糊的"适量"、"酌情"。
   - **输出含义**：返回值的单位和物理量。
   - **随机性声明**（若适用）：明确说明哪些随机源受 seed 控制（如 "demand noise is seed-controlled via `DYNAMIC_ARGS['seed']`"）。

2. **语言风格**：
   - 用陈述句，避免问句或祈使句。
   - 使用 "is/equals/returns" 这类确定词汇，避免 "may/might/could"。
   - 数字和单位必须明确（"per day", "per unit", "per period"）。
...
def store_demand(period: int, product_id: int) -> float:
    """Demand function for stores. Has seasonal and promo effects."""
    ...

# ✅ 合格 — 精确、有数字、有条件、注明随机性
def store_demand(period: int, product_id: int) -> float:
    """Product-1 demand equals baseline plus weekly pulse plus optional promotion uplift for product 1.
    Weekend (days 6, 0 of 7-day cycle) adds 4.0 extra demand. Promotion window (arg:promo_start to arg:promo_end)
    adds arg:promo_lift extra demand. Result is max(0.0, base + weekly_pulse + promo). Returns 0 for non-product-1."""
    ...
```

4. **与 BLUEPRINT_TEXT_GUIDELINES 的关系**：详见的 `stockpyl_cases/BLUEPRINT_TEXT_GUIDELINES.md` §4 "Hook Docstring Rules" 和 §8 "Stochastic Scenario Rules"。
...
**随机场景额外字段**（仅当场景含随机逻辑时使用）：

* `seed_mode` (str): 可选。`"incremental"`（推荐）表示每次 run 种子递增，`"fixed"` 表示所有 run 使用相同种子。
* `seed_start` (int): seed 起始值。`incremental` 模式下，run_i 使用 `seed_start + i`。
* `seed_arg` (str): CLI 中 seed 参数名，如 `"seed"`（默认）或 `"random_seed"`。
...
test_cases = [
    {
        ...
        "seed_start": 2026,
        "seed_arg": "seed",
        "cli_kwargs": {"num_stores": 5, "base_demand": 20.0, "seed": 2026},
        "stdin_payload": "",
    }
]
...
test_cases = [
    {
        ...
        "runs": 30,
        "oracle_runs": 500,
        "cli_kwargs": {"num_retailers": 3, "retailer_holding_cost": 2.5},
        "stdin_payload": "30.0\n30.0\n",
    }
]
```
