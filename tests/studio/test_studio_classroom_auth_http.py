...
class StudioClassroomAuthHttpTests(unittest.TestCase):
    ...
    def test_registration_and_private_conversation_workspace_scope(self) -> None:
        ...
        self.assertIn(b'auth-shell auth-shell-register', page)
        self.assertIn(b'auth-register-grid', page)
        self.assertIn(b"Invitation-only access", page)
        self.assertIn(b"12+ characters", page)
        self.assertIn(b"Methods propose. Environments evaluate.", page)
        self.assertIn(b"OptPilot connects the loop and keeps the evidence.", page)
        self.assertIn(b"Retain evidence", page)
