...
class StudioWorkspaceLifecycleStaticTest(unittest.TestCase):
    ...
    def test_sidebar_workspace_controls_stay_inside_the_clipped_rail(self) -> None:
        ...
        self.assertIn("grid-template-columns: minmax(0, 1fr)", header)
        self.assertIn("padding: 6px 8px", header)
        self.assertIn("display: flex", header_actions)
        self.assertIn("flex-wrap: wrap", header_actions)
        self.assertIn("justify-content: flex-start", header_actions)
        self.assertIn("width: 100%", workspace_list)
        self.assertIn("min-width: 0", workspace_list)
        self.assertIn("grid-template-columns: minmax(0, 1fr) 28px", card)
        self.assertIn("width: 100%", card)
        self.assertIn("padding: 0", session_main)
        self.assertIn("position: static", more)
        self.assertIn("grid-column: 2", more)
        self.assertIn("height: 28px", more)
        self.assertIn("height: 28px", more_open)
        self.assertNotIn(
            ".sidebar-workspaces .session-card-more:not([open]) {",
            self.styles,
        )
        self.assertIn("position: static", more_summary)
        ...
        self.assertIn("border-color:", open_summary)
        self.assertIn("width: min(210px, calc(100% - 8px))", card_actions)
        self.assertIn("position: absolute", card_actions)
        self.assertIn("right: 0", card_actions)
