...
The remaining gaps are production hardening for the OpenHands HTTP bridge, safe
file and shell tools scoped to attached workspaces, streaming tool/event updates,
richer workspace tooling, and production isolation for users, secrets, and
quotas.
...
- **Catalog**: reusable registered environments, methods, and resources.
...
Configure assistant-facing capabilities from the assistant settings gear. These
capabilities and context packages are
not OptPilot-run study components. They include MCP servers, OpenHands custom
tools, AgentSkills, knowledge bases, reusable prompt/instruction bundles,
visualization launchers, dataset profilers, and repository-backed utilities such
as `user_catalog/resources/devs_display_new/`.
...
- assistant capability inspection workspace
...
Code Server command URI, or a small local bridge if Code Server supports the needed
file-focus behavior reliably.

Do not build a custom dirty-file detector in OptPilot. Code Server provides
browser-based editor behavior, including Hot Exit behavior
for unsaved changes.
OptPilot should avoid forcing iframe unloads when possible and should rely on
Code Server for editor-close or browser-close handling. If the user
...
1. **Discover configs**
   ...
   `apiVersion: optpilot.io/v1` and `config: environment | method`.
   Study YAMLs may still be discovered for validation or launch, but they are not
...
3. **Validate**
   Run `validate_authoring_config` on each selected environment or method config.
...
- saved study-plan YAMLs binding entries inside the same repo

The catalog should treat each valid environment, method, or resource manifest as
one catalog entry. Saved study-plan YAMLs remain reusable files, but they are not
registered catalog entries. Entries can share a `workspace_root` and
`source_repository`.
...
Assistant capabilities are reusable skills, servers, tools, and resource-backed
helpers that help the assistant do work, but are not themselves environment,
method, or study entries.
...
- local scripts, CLIs, notebooks, or simulator generators imported into
  `user_catalog/resources/<id>/`

Assistant Settings should let users inspect, test, enable, and disable these
capabilities. Enabling means "make this available to the assistant with clear
usage instructions and safety policy"; it does not mean "copy into the
environment/method catalog."

### Capability Data Shape

Represent capability records with small manifests:
...
```json
{
  ...
  "source": {
    "type": "local_path",
    "path": "user_catalog/resources/devs_display_new"
  },
    ...
    "commands": [
      {
        ...
        "cwd": "user_catalog/resources/devs_display_new",
  ...
  "agent_exposure": {
    ...
    "allowed_roots": ["user_catalog/resources/devs_display_new"],
...
### Assistant Capability Registration Flow

Assistant Settings should capture capability records instead of registering them
as OptPilot catalog configs:
...
2. **Inspect**
   Let the assistant or user summarize what the capability does, what files are
...
5. **Enable**
   Store the capability record, add it to the current user's enabled assistant
   capability set, and include compact summaries in future assistant context
   packets.

### Assistant Capabilities And OpenHands

OpenHands should see configured capabilities through an OptPilot-controlled layer,
not by receiving unrestricted filesystem or network access.

Use this mapping:

| Capability type | OpenHands exposure |
| --- | --- |
...
| AgentSkill | Load as `SKILL.md` progressive-disclosure context for sessions where the skill is enabled |
...
For unofficial tools imported from ignored local source folders, do not require
them to be rewritten as MCP servers before they are useful. Keep the local source
folder as import material, then register the durable copy under
`user_catalog/resources/<id>/` and pair it with an AgentSkill or custom-tool
...
Then OpenHands can use the capability in two ways:

1. The context packet tells the agent the capability exists and summarizes when to
   use it.
2. OptPilot exposes controlled capability tools that inspect records and later
   run only declared command templates inside declared roots.
...
GET  /api/agent/capabilities/{kind}/{id}
```

Any future execution endpoint should accept only a registered command id plus
structured arguments. Avoid accepting arbitrary shell strings from the browser
or agent.

### Assistant Capabilities In Assistant Context

Add enabled assistant capabilities to the context packet:

```json
{
  "assistant_capabilities": {
    "counts": {"skills": {"enabled": 1}, "mcp_servers": {"enabled": 1}},
    "enabled": {"skills": [{"id": "devs-display-new"}]},
...
call a capability detail or retrieval tool when it needs detailed docs.
...
- Studies are saved, validated, and launched from their existing YAML path; they
  do not have a registration target.
...
workspace runtime containers publish code-server on loopback ports
```

Production:

- OptPilot API/UI service per user or tenant.
- per-workspace runtime containers managed by OptPilot, each running Code Server
  when the workspace is opened in the editor.
- OpenHands agent server either shared behind OptPilot, per user, or per active
  conversation depending on the trust boundary.
- Workspace roots mounted into their runtime container with role-appropriate
  read-write or read-only permissions. OpenHands accesses workspaces through
  OptPilot tools, not by directly owning the workspace volume.
...
| `optpilot_catalog_detail` | inspect a selected catalog entry or saved study plan contract |
| `optpilot_capability_list` | list configured assistant skills, MCP servers, custom tools, and permissions |
| `optpilot_capability_detail` | inspect one configured capability record |
...
```json
{
  ...
  "assistant_capabilities": {"counts": {"skills": {"enabled": 1}}},
  "available_tools": ["optpilot_config_validate", "optpilot_registration_prepare", "optpilot_capability_list"]
}
```
...
- when using assistant capabilities, inspect the capability record first, respect
  declared allowed roots and commands, and prefer wrappers over arbitrary shell
  commands
...
- if it produces HTML, images, plots, notebooks, CSV, JSON, SQLite, logs, or
  videos, let Code Server preview those artifacts with built-in viewers
...
1. **OpenHands event streaming**
   The HTTP dispatch bridge and conversation resume path are in place. Continue
   hardening event translation, live streaming, and approved OptPilot tool
   execution through the platform boundary.
...
3. **Specialized capabilities**
   Expand Assistant Settings and capability tools for resource-backed utilities
   such as `user_catalog/resources/devs_display_new/`, GitHub import helpers,
   dataset profiling, knowledge bases, MCP servers, and visualization launchers
   behind the same permission model.
...
- mocked assistant-capability test for context-only, MCP-style, and
  repository-backed capabilities
- capability safety test: executable wrappers reject undeclared commands and
  paths outside the capability allowlist
...
- OptPilot should rely on Code Server for unsaved editor buffers instead
...
- Visualizations should be launched as workspace preview tools, with web URLs as
  the preferred path and Code Server viewers or remote-display adapters
  for non-web outputs.
- Assistant capabilities are not environment/method/study catalog entries.
  Formal MCP tools, OpenHands custom tools, AgentSkills, knowledge bases, and
  unofficial repository-backed resources should be represented through Assistant
  Settings plus the same permission model.
