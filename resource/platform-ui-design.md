...
OptPilot Studio
  ...
  Assistant settings
    OpenHands runtime, AgentSkills, MCP servers, custom tools, permissions,
    Docker/Podman status, and LLM credentials
  Assistant control
...
After intake or catalog inspection, the UI opens a Code Server editor with an
assistant panel:
...
The default model is one workspace runtime container per workspace. Code Server
runs inside that workspace container, so the editor terminal, assistant shell
tools, and user dependency installs share the same filesystem and runtime.
...
- Multiple environments, methods, and plans can be open at once as separate
  workspaces, each with its own runtime container.
- Workspace runtime lifecycle, mounts, and deletion semantics are defined in
  `resource/studio-folder-management.md`.
...
Specialized generators should be assistant capabilities available to the general
coding agent, not top-level product choices. For example,
`user_catalog/resources/devs_display_new/` can be registered as a reusable
resource with an associated skill, MCP server, or OpenHands custom tool that the
agent may use when the request needs discrete-event simulator generation. The same
mechanism can support MCP tools, AgentSkills, knowledge bases, repo inspection,
dependency detection, adapter writing, dataset profiling, smoke-test execution,
preview launching, and future domain-specific generators.
...
An assistant capability does not need to be a formal MCP server or OpenHands
custom tool on day one. It can begin as a catalog resource plus an AgentSkill
that explains when to use it, which docs to read, which commands are safe to
run, and which paths the agent may access. Formal MCP/OpenHands wrappers can be
added later without changing the user-facing concept.
...
- saved study YAML files in draft workspaces or the launch-selected path
...
| --- | --- |
| Catalog service | scan, index, create, clone, inspect, and register environment/method/resource entries |
...
- Add installed capability configuration for `user_catalog/resources/devs_display_new/`.
