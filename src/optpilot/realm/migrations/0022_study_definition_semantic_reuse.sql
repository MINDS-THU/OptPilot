-- A run-definition digest identifies immutable execution semantics, not an
-- authorization owner.  Equivalent definitions may therefore be retained by
-- independent principals or independent user-requested launches.  Keep owner
-- identity as the manifest key and use the semantic digest only as a lookup
-- index.  Content refs remain shared immutable references; this change does
-- not copy content-store objects.
PRAGMA defer_foreign_keys = ON;

-- SQLite validates every retained trigger when the replacement table is
-- renamed.  Temporarily remove the two cross-table guards that name the
-- manifest table; their definitions are restored byte-for-byte below.
DROP TRIGGER study_definition_ref_requires_open_manifest;
DROP TRIGGER study_definition_revision_zero_requires_complete_definition;

CREATE TABLE study_definition_manifests_v22 (
...
CREATE TABLE study_definition_refs_v22 (
    owner_id TEXT NOT NULL
        REFERENCES study_definition_manifests_v22(owner_id),
...
INSERT INTO study_definition_manifests_v22(
    owner_id,
    owner_revision,
    manifest_digest,
    owner_derivation_manifest_digest,
...
SELECT
    ...
    manifest_json,
    created_txn_id
FROM study_definition_manifests;

INSERT INTO study_definition_refs_v22(
    owner_id,
    semantic_role,
    store_id,
    content_ref,
...
SELECT
    ...
    content_ref,
    created_txn_id
FROM study_definition_refs;

DROP TABLE study_definition_refs;
DROP TABLE study_definition_manifests;
ALTER TABLE study_definition_manifests_v22
RENAME TO study_definition_manifests;
ALTER TABLE study_definition_refs_v22
RENAME TO study_definition_refs;

CREATE INDEX study_definition_manifests_run_definition_digest_index
ON study_definition_manifests(run_definition_digest);
