...
CREATE TABLE provider_trust_decisions (
    ...
    contract TEXT NOT NULL CHECK(
        contract = 'optpilot-stdlib-gateway-v1'
    ),
    ...
    created_txn_id INTEGER NOT NULL UNIQUE REFERENCES ledger_transactions(txn_id),
    created_at REAL NOT NULL,
    UNIQUE(policy_id, image_ref, sequence),
    UNIQUE(policy_id, image_ref, decision_id),
    FOREIGN KEY(policy_id, image_ref, previous_decision_id)
        REFERENCES provider_trust_decisions(policy_id, image_ref, decision_id),
    CHECK(
...
CREATE TABLE provider_trust_heads (
    ...
    PRIMARY KEY(policy_id, image_ref),
    FOREIGN KEY(policy_id, image_ref, decision_id)
        REFERENCES provider_trust_decisions(policy_id, image_ref, decision_id)
);

CREATE INDEX provider_trust_decisions_history_index
ON provider_trust_decisions(policy_id, image_ref, sequence);
