...
from factory_sim.simulation.entities.conveyor import Conveyor, TripleBufferConveyor, BaseConveyor
from factory_sim.simulation.entities.warehouse import Warehouse, RawMaterial
from factory_sim.simulation.entities.station import Station
from factory_sim.simulation.entities.agv import AGV
from factory_sim.simulation.entities.quality_checker import QualityChecker
from factory_sim.game_logic.order_generator import OrderGenerator
from factory_sim.game_logic.fault_system import FaultSystem
from factory_sim.game_logic.kpi_calculator import KPICalculator
...
class Line:
    ...
    def __init__(self, env: simpy.Environment, line_name: str, line_config: Dict, 
                 mqtt_client: MQTTClient, database: SimulationDatabase, topic_manager: TopicManager,
    ...
    def _create_devices(self):
        ...
        for station_cfg in self.config.get('stations', []):
            if station_cfg['id'] == 'QualityCheck':
                station = QualityChecker(env=self.env, mqtt_client=self.mqtt_client, database=self.database, topic_manager=self.topic_manager, line_id=self.name, **station_cfg)
            else:
                station = Station(env=self.env, mqtt_client=self.mqtt_client, database=self.database, topic_manager=self.topic_manager, line_id=self.name, **station_cfg)
        ...
        for agv_cfg in self.config.get('agvs', []):
            ...
            agv = AGV(env=self.env, mqtt_client=self.mqtt_client, database=self.database, topic_manager=self.topic_manager, 
                     fault_system=self.fault_system, kpi_calculator=self.kpi_calculator,
                     line_id=self.name, agv_operations=agv_operations, 
        ...
        for conveyor_cfg in self.config.get('conveyors', []):
            ...
            common_args = {
                ...
                "transfer_time": conveyor_cfg['transfer_time'], "mqtt_client": self.mqtt_client, "database": self.database,
    ...
    def _create_game_logic_systems(self):
        ...
        if 'fault_system' in self.config and not self.no_faults_mode:
            ...
            self.fault_system = FaultSystem(self.env, self.all_devices, self.mqtt_client, self.database, self.topic_manager, self.name, kpi_calculator=self.kpi_calculator, **fs_config)
