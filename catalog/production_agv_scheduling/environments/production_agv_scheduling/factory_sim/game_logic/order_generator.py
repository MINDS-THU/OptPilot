...
from factory_sim.config.schemas import NewOrder, OrderItem, OrderPriority
from factory_sim.config.topics import NEW_ORDER_TOPIC
from factory_sim.utils.mqtt_client import MQTTClient
from factory_sim.simulation.entities.warehouse import RawMaterial
...
class OrderGenerator:
    ...
    def __init__(self, env: simpy.Environment, raw_material: RawMaterial, mqtt_client: Optional[MQTTClient] = None, database: Optional[SimulationDatabase] = None, topic_manager: Optional[TopicManager] = None, kpi_calculator: Optional[KPICalculator] = None, **kwargs):
        self.env = env
    ...
    def _publish_order(self, order: NewOrder):
        ...
        try:
            if self.database:
                ...
                order_dict['timestamp'] = order_dict.pop('created_at')
                self.database.insert_data('order', order_dict)
        except Exception as e:
            print(f"[{self.env.now:.2f}] ❌ Failed to insert new order into database: {e}")