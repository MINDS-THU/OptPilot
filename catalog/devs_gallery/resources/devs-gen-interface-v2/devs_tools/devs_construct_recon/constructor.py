...
class DEVSConstructRecon(Tool):
    ...
    def build_from_plan(
        ...
        *,
        ...
        target_log_dir = self.log_dir_path
        detailed_planning_llm_summary: Dict[str, Any] = {"total_calls": 0}
        detailed_architecture_started_at = time.time()
        detailed_architecture_finished_at = detailed_architecture_started_at
        logs_dir = self.working_directory / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        detailed_timing_log = logs_dir / "timing_debug.jsonl"
        self.timing_log_file = detailed_timing_log
        detailed_timing_log.write_text(
            json.dumps(
                {
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="devs-detail-") as temporary_dir:
                ...
                detailed_architecture_finished_at = time.time()
                self._log_timing(
                    "Detailed Architecture Complete",
                    detailed_architecture_started_at,
                    detailed_architecture_finished_at,
                )
                detailed_planning_llm_summary = get_llm_logger().get_summary()
        ...
        try:
            ...
            self.build_logger.save_stage_result(
                "detailed_planning_llm_summary",
                detailed_planning_llm_summary,
                "detailed_planning_llm_summary.json",
            )
            self.build_logger.log_stage(
...
"""
    ...
    def _phase2_construct_code_recursive(self, node: PlanTreeNode, skip_simulation_check: bool, depth: int, only_ensure_executable: bool) -> StandardContextModel:
        ...
        if node.children:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(self.concur_num, self.max_workers)) as executor:
                futures = [
                    executor.submit(contextvars.copy_context().run, build_single_child, child)
                    for child in node.children
                ]
    ...
    def _sanitize_name(self, name: str) -> str:
        ...
        parts = [part for part in re.split(r"[^0-9a-zA-Z]+", raw_name) if part]
        if not parts:
            return "Model"

        # The root name is used as a Python class name and is also passed to
        # the global planner.  Keeping snake_case here makes the prompt
        # contradictory: it asks for the exact root name and for PascalCase
        # module names.  Normalize only the class name; the project folder is
        # canonicalized separately and remains unchanged.
        if len(parts) > 1 or not raw_name[:1].isupper():
            name = "".join(part[:1].upper() + part[1:] for part in parts)
        else:
            name = re.sub(r"[^0-9a-zA-Z]", "", raw_name)
