...
interface Props {
  ...
  active: boolean;
  onTrace?: (type: string, payload?: Record<string, unknown>) => void;
}
...
export const SimulationRunPanel: React.FC<Props> = ({
  ...
  const trace = useCallback((type: string, payload: Record<string, unknown> = {}) => {
    onTrace?.(type, {
      simulation_id: simulationId,
      simulation_name: simulationName,
      run_id: runRef.current?.run_id || null,
      ...payload,
    });
  }, [onTrace, simulationId, simulationName]);

  const traceFrontendError = useCallback((
    where: string,
    reason: unknown,
    ...
    const message = reason instanceof Error
      ? reason.message
      : String((reason as { message?: unknown } | null)?.message || reason || 'Unknown frontend error');
  ...
  const loadSpec = async () => {
    ...
    try {
      ...
      setSpec(null);
      if (!missing) traceFrontendError('SimulationRunPanel.loadSpec', reason);
    } finally {
  ...
  }, [scopeKey, simulationStatus, simulationVersion]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        traceFrontendError('SimulationRunPanel.pollRun', reason, { run_id: runId });
      }
  ...
  useEffect(() => {
    ...
    void getSimulationResultPreview(
      ...
      traceFrontendError('SimulationRunPanel.loadEventTrace', reason, {
        run_id: run.run_id,
        file_path: eventTraceFile.path,
        file_size: eventTraceFile.size ?? null,
      });
    });
  ...
  const start = async () => {
    ...
    try {
      ...
      trace('simulation_run_clicked', {
        run_id: next.run_id,
        argument_names: Object.keys(parsedValues).sort(),
      });
    } catch (reason: any) {
      if (scopeKeyRef.current !== requestScope) return;
      setError(reason?.message || 'The simulation could not be started.');
      traceFrontendError('SimulationRunPanel.start', reason, {
        argument_names: Object.keys(parsedValues).sort(),
      });
    } finally {
  ...
  const stop = async () => {
    ...
    try {
      ...
      runRef.current = next;
      setRun(next);
      trace('simulation_stop_clicked', { run_id: runId });
    } catch (reason: any) {
      if (scopeKeyRef.current !== requestScope) return;
      setError(reason?.message || 'The simulation could not be stopped.');
      traceFrontendError('SimulationRunPanel.stop', reason, { run_id: runId });
    } finally {
  ...
  const showObservationInReplay = (eventIndex: number) => {
    trace('replay_seeked', {
      ...
      event_count: eventTrace?.events.length || null,
    });
  ...
  const downloadResult = async (file: SimulationResultFile) => {
    ...
    try {
      ...
      traceFrontendError('SimulationRunPanel.downloadResult', reason, {
        file_path: file.path,
        file_size: file.size ?? null,
        run_id: run.run_id,
      });
  ...
  const openResult = async (file: SimulationResultFile) => {
    ...
    setResultFileError(null);
    try {
      const preview = await getSimulationResultPreview(
        sessionId,
        simulationId,
        run.run_id,
        file.path
      ...
      traceFrontendError('SimulationRunPanel.openResult', reason, {
        file_path: file.path,
        file_size: file.size ?? null,
        run_id: run.run_id,
      });
              ...
              {terminalStatuses.has(run.status) && eventTraceStatus === 'loaded' && eventTrace && eventTrace.events.length > 0 && graphNodes.length > 0 && (
                <div ref={replayRegionRef} tabIndex={-1} className="scroll-mt-4 focus:outline-none" aria-label="Behavior replay">
                  <BehaviorReplay
                    ...
                    active={active}
                    onTrace={(type, payload = {}) => trace(type, {
                      run_id: run.run_id,
                      event_count: eventTrace.events.length,
                      ...payload,
                    })}
                  />
                </div>
