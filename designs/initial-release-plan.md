...
**U7 — Retire the legacy shell (M).** `?shell=legacy`, the `legacy-navigation` markup, legacy assistant session cards, and `_shortlist_card_legacy_ui_projection` double the render paths. Remove after U1–U3 land; switch the no-hash route fallback from `#/workspaces` to `#/conversations` at the same time.
