...
Site access and participant identity are separate. The optional password is a
lightweight shared access gate. In the default `cookie` identity mode, the
backend issues each browser profile a signed, pseudonymous
`devs_trial_identity` cookie and binds every session to that identity. Managed
OptPilot Interfaces use `launch` mode instead: one owner is derived from the
launch-owned runtime root and no participant cookie is sent. All session-scoped
routes enforce the selected ownership boundary.
...
| `DEVS_DISPLAY_IDENTITY_MODE` | Session ownership boundary: `cookie` (default) or `launch`. Invalid values fail startup. |
| `DEVS_DISPLAY_PARTICIPANT_COOKIE_TTL_SECONDS` | Participant cookie lifetime. Defaults to 30 days. |
| `DEVS_DISPLAY_PARTICIPANT_COOKIE_SECRET` | Optional participant-token secret. Otherwise an owner-only key is created under the data root. |
| `DEVS_DISPLAY_PARTICIPANT_COOKIE_SECURE` | Force the Cookie `Secure` attribute on or off. By default it follows the request scheme. |

When no password variable is configured, shared-password authentication is
disabled for local development. Session ownership remains enabled. Browser
requests must include credentials when password authentication is active; the
supplied frontend does this for every backend request.
...
When password authentication is enabled, all other backend endpoints require:
...
- One participant can run at most one generation request across all sessions.
- Read APIs for the same session can run while a chat request is running.
- `DEVS_DISPLAY_MAX_ACTIVE_GENERATIONS` bounds the global worker pool and
  therefore the number of simultaneous model-generation/finalizer pipelines.
...
Creates a new session workspace owned by the signed participant cookie. A
browser-supplied `client_id` is ignored and cannot choose the owner. Every
source session referenced by `clone_projects` must have the same owner.
...
Lists recent sessions owned by the signed participant cookie. `client_id` and
`include_all` query parameters are not accepted as authorization.
