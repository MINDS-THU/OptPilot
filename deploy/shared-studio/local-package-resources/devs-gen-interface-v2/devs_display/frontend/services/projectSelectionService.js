 ...
 * forever. Re-read when that same simulation leaves `updating`, or when its
 * version changes in place after files are added or repaired; changing
 * simulations is already handled by the panel's normal scope reset.
 *
 * @param {{scopeKey: string, status?: string | null, version?: number | null} | null} previous
 * @param {{scopeKey: string, status?: string | null, version?: number | null}} current
 * @returns {boolean}
 */
export const shouldRefreshSimulationSpecAfterProjectUpdate = (
  ...
  if (!previous || previous.scopeKey !== current.scopeKey) return false;
  if (previous.status === 'updating' && current.status !== 'updating') return true;
  return (
