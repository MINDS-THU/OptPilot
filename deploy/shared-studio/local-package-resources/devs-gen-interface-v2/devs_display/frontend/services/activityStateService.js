...
const TERMINAL_FAILURE_KEYS = new Set(['request_failed']);

/**
 * A failed activity can be an intermediate check that the generator will
 * repair. Reserve the red failure mark for terminal request/finalizer
 * failures; all other failed activities remain recoverable warnings.
 *
 * @param {string} state
 * @param {string} [activityKey]
 * @returns {'none' | 'recoverable' | 'terminal'}
 */
export const classifyActivityFailure = (state, activityKey = '') => {
  if (state !== 'failed') return 'none';
  if (
    TERMINAL_FAILURE_KEYS.has(activityKey)
    || activityKey === 'codex_finalizer'
    || activityKey.startsWith('codex_finalizer:')
  ) {
    return 'terminal';
  }
  return 'recoverable';
