...
test('refreshes the runner when the selected simulation version changes in place', () => {
  assert.equal(
    shouldRefreshSimulationSpecAfterProjectUpdate(
      { scopeKey: 'session-a\0restaurant', status: 'ready', version: 2 },
      { scopeKey: 'session-a\0restaurant', status: 'ready', version: 3 }
    ),
    true
  );
});
