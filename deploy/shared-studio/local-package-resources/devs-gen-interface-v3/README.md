# DEVS Simulation Generator Interface v3
...
- `devs_tools/devs_construct_recon_v3/`: Active parallel DEVS construction engine and Interface adapter.
- `devs_tools/devs_construct_recon/`: Stable Interface, execution, and result-contract helpers retained for compatibility.
...
### Standalone participant identity and capacity

The standalone server assigns each browser profile a signed, pseudonymous
participant cookie. The cookie is `HttpOnly`, lasts 30 days by default, and is
backed by `participants.sqlite3` plus an owner-only signing key under
`DEVS_INTERFACE_DATA_ROOT`. Session ownership is checked on every
`/sessions/{session_id}` route; browser-supplied client ids and `include_all`
queries do not grant access. Closing a tab or browser therefore does not lose a
student's session list, while another browser profile cannot read it.

The standalone deployment also bounds shared resources:

- `DEVS_DISPLAY_MAX_ACTIVE_GENERATIONS` controls the global generation worker
  pool (shared-deployment default: 3).
- `DEVS_INTERFACE_CONCURRENCY` controls component-generation concurrency inside
  one request (shared-deployment default: 1).
- `DEVS_DISPLAY_PARTICIPANT_DATA_MAX_BYTES` limits all workspaces owned by one
  participant (shared-deployment default: 200 MiB).
- `DEVS_INTERFACE_DATA_MAX_BYTES` remains the whole-service storage ceiling
  (shared-deployment default: 20 GiB).

One participant may have only one active generation request across all of
their sessions. Requests from different participants wait when the global
worker pool is full; a second active request from the same participant is
rejected with a clear retry message. Simulation execution evidence is already
pruned to a bounded retained set. Use HTTPS in a public deployment so the
participant cookie can also carry the `Secure` attribute.

Managed OptPilot launches set `DEVS_DISPLAY_IDENTITY_MODE=launch`. Each launch
already has a separate writable runtime root, so the backend derives one stable
owner from that root and does not create or send participant cookies. This
prevents the browser's host-wide cookie jar from mixing interfaces exposed on
different presentation ports. The default remains `cookie` for standalone
deployments; an unsupported mode fails during backend startup.

### Durable session collector

Set both `DEVS_COLLECTOR_URL` and `DEVS_COLLECTOR_INGEST_TOKEN` to retain
pseudonymous sessions, requests, operation traces, ratings, and sanitized model
snapshots outside the Interface runtime. `DEVS_COLLECTOR_SOURCE` distinguishes
deployments such as `standalone` and `optpilot`. Reporting is asynchronous and
coalesced per session: a slow or unavailable collector is written to the
private backend log but never delays generation, simulation, or feedback.

If either required value is absent, reporting is disabled and the Interface
