...
export const downloadSessionProjectArchive = async (
    sessionId: string,
    projectId: string
): Promise<Blob> => {
    const token = getStoredAuthToken();
    const response = await fetch(
        `${AGENT_API_URL}/sessions/${encodeURIComponent(sessionId)}/projects/${encodeURIComponent(projectId)}/archive`,
        {
            credentials: 'include',
            headers: {
                ...(token ? { 'Authorization': `Bearer ${token}` } : {})
...
export const recordSessionTrace = async (
    sessionId: string,
    type: string,
    input: {
        actor?: 'user' | 'system' | 'frontend';
    ...
    const data = await jsonFetch<{ trace: TraceEvent }>(`/sessions/${encodeURIComponent(sessionId)}/trace`, {
        method: 'POST',
        body: JSON.stringify({
            type,
            actor: input.actor || 'frontend',
...
export const submitSessionEvaluation = async (
    sessionId: string,
    input: {
    ...
    const data = await jsonFetch<{ evaluation: EvaluationRecord }>(
...
export const getSessionEvaluations = async (
    ...
    const data = await jsonFetch<{ evaluations: EvaluationRecord[] }>(
        `/sessions/${encodeURIComponent(sessionId)}/evaluations?limit=${encodeURIComponent(String(limit))}`
    );
