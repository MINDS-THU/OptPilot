...
class BackendServiceTests(unittest.TestCase):
    ...
    def test_collector_bundle_is_pseudonymous_and_excludes_private_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            owner = "participant_44444444444444444444444444444444"
            session, _ = service.create_session("Collector test", [], owner)
            ...
            project_root = Path(
                service._project_abs_path(project, session["session_id"])
            )
            (project_root / ".env").write_text("API_KEY=private\n", encoding="utf-8")
            analysis = project_root / "_analysis_logs"
            analysis.mkdir()
            (analysis / "llm_input.txt").write_text("private", encoding="utf-8")
            (analysis / "detailed_planning_llm_summary.json").write_text(
                json.dumps(
                    {
                        "calls": [
                            {
                                "phase": "phase1b_detailed_plan_atomic",
                                "model": "example/model",
            ...
            bundle = service._collector_bundle(session["session_id"], True)

            self.assertEqual(bundle["participant_id"], owner)
            self.assertNotIn("workspace_path", bundle["session"])
            self.assertEqual(len(bundle["projects"]), 1)
            archive_bytes = base64.b64decode(
                bundle["projects"][0]["snapshot_base64"]
            )
            with zipfile.ZipFile(io.BytesIO(archive_bytes)) as archive:
                names = archive.namelist()
            self.assertIn("queue_model/run.py", names)
            self.assertFalse(any("_analysis_logs" in name for name in names))
            self.assertFalse(any(name.endswith("/.env") for name in names))
            llm_usage = bundle["projects"][0]["project"]["llm_usage"]
            self.assertEqual(llm_usage["total_tokens"], 12)
            self.assertEqual(llm_usage["models"][0]["model"], "example/model")
            self.assertEqual(llm_usage["calls"], 1)
            serialized_usage = json.dumps(llm_usage)
            self.assertNotIn("SecretComponent", serialized_usage)
            self.assertNotIn("private error text", serialized_usage)

    def test_listing_sessions_does_not_touch_updated_at_when_project_count_is_unchanged(self):
        with tempfile.TemporaryDirectory() as tmp:
            write_project(tmp, "stable_project")
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            ...
            self.assertEqual(second["project_count"], first["project_count"])
            self.assertEqual(second["updated_at"], first_updated_at)

    def test_participant_listing_does_not_scan_another_owners_workspace(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            alice, _ = service.create_session(
                "Alice",
                [],
                "participant_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            )
            bob, _ = service.create_session(
                "Bob",
                [],
                "participant_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
            )
            synchronized = []
            original = service._sync_session_projects

            def record_sync(session_id):
                synchronized.append(session_id)
                return original(session_id)

            with patch.object(
                service,
                "_sync_session_projects",
                side_effect=record_sync,
            ):
                sessions = service.list_sessions(
                    client_id="participant_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
                )
            ...
            self.assertIn(alice["session_id"], synchronized)
            self.assertNotIn(bob["session_id"], synchronized)

    def test_one_participant_cannot_queue_generations_in_two_sessions(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first, _ = service.create_session(
                "First design", [], "participant_11111111111111111111111111111111"
            )
            second, _ = service.create_session(
                "Second design", [], "participant_11111111111111111111111111111111"
            )
            service.submit_chat(
                first["session_id"],
            ...
            with self.assertRaisesRegex(
                RuntimeError,
                "active generation request in another design",
            ):
                service.submit_chat(
                    second["session_id"],
    ...
    def test_participant_storage_limit_blocks_additional_upload(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            owner = "participant_22222222222222222222222222222222"
            session, _ = service.create_session("Limited design", [], owner)
            used_bytes = service.participant_storage_status(owner)[
                "participant_used_bytes"
            ]
            service.participant_data_max_bytes = used_bytes + 4
    ...
    def test_cancelling_queued_request_releases_session_agent(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(
                DummyAgent(),
                tmp,
            ...
            session, _ = service.create_session(
                "Disposable agent",
                [],
                "participant_33333333333333333333333333333333",
            )
            workspace = service._session_workspace(session["session_id"])
            service._agent_for_workspace(workspace)
            ...
            self.assertIn(workspace, service.workspace_agents)

            service.cancel_request(session["session_id"], request["request_id"])

            self.assertNotIn(workspace, service.workspace_agents)
    ...
    def test_codex_acceptance_gate_detects_unfinished_delivery_flow(self):
        stdout = "\n".join(
            json.dumps(
                {
        ...
        issues = DEVSBackendService._codex_acceptance_issues(
            {"status": "succeeded", "stdout": stdout, "arguments": {}}
        )

        self.assertTrue(any("delivery has" in issue for issue in issues))

    def test_codex_acceptance_gate_allows_completed_delivery_flow(self):
        rows = []
        for index in range(1, 12):
            rows.append(
                {
            ...
            rows.append(
                {
                    "event": "delivery_completion",
                    "order_id": index,
                    "courier_id": index % 3,
                    "time": float(index * 3 + 2),
                }
            )
    ...
    def test_chat_auto_check_defaults_on(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            session_id = current_session_id(service)
            request, _ = service.submit_chat(
                ...
                None,
                False,
                "auto-check-default-key",
            )

            self.assertTrue(request["auto_check"])
            persisted = service.get_request(session_id, request["request_id"])
            self.assertTrue(persisted["auto_check"])

    def test_auto_check_off_preserves_failure_repair_when_finalizer_is_enabled(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(
            os.environ,
            {
                "DEVS_DISPLAY_CODEX_FINALIZER": "1",
                "DEVS_DISPLAY_AUTOMATIC_REPAIR_ATTEMPTS": "1",
            },
        ):
            root = Path(tmp)
            ...
            self.assertEqual(finished["status"], "completed", finished)
            self.assertFalse(finished["auto_check"])
            self.assertEqual(len(agent.prompts), 2)
            events = service.get_events(
                session_id, request_id=request["request_id"]
            )["events"]
            self.assertNotIn(
                "codex_finalizer",
                [
                    event.get("activity_key", "").split(":")[0]
                    for event in events
                ],
            )
            self.assertIn(
    ...
    def test_finalizer_timeout_after_successful_revalidation_publishes_bundle(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(
            os.environ,
            {
                "DEVS_DISPLAY_CODEX_FINALIZER": "1",
                "DEVS_DISPLAY_CODEX_FINALIZER_DRIVER": "codex_cli",
                "DEVS_DISPLAY_AUTOMATIC_REPAIR_ATTEMPTS": "0",
            },
        ):
            root = Path(tmp)
            ...
            def timeout_after_repair(**kwargs):
                bundle_root = Path(kwargs["bundle_root"])
                (bundle_root / "run.py").write_text(
                    "print('repaired by finalizer')\n",
                    encoding="utf-8",
                )
                raise RuntimeError("Codex CLI finalizer timed out after 900 seconds.")

            service._run_codex_cli_finalizer = timeout_after_repair
            session_id = current_session_id(service)
            request, _ = service.submit_chat(
                session_id,
                "Create and auto-check a repairable model",
                None,
                False,
                "finalizer-timeout-revalidation-key",
            )
            for _ in range(80):
                finished = service.get_request(session_id, request["request_id"])
                if finished["status"] in {"completed", "failed", "cancelled"}:
            ...
            self.assertEqual(
                validation["codex_finalizer"]["verdict"],
                "inconclusive",
            )
            self.assertIn(
                "passed the backend smoke test",
                validation["message"],
            )

    def test_codex_cli_finalizer_prefers_prepared_python(self):
        with patch.dict(
            os.environ,
            {"OPTPILOT_INTERFACE_PYTHON": "/tmp/prepared runtime/bin/python"},
            clear=False,
        ):
            instruction = DEVSBackendService._codex_cli_run_instruction(
                "generated bundle"
            )

        self.assertIn("prepared interface Python", instruction)
        self.assertIn(
            "cd 'generated bundle' && '/tmp/prepared runtime/bin/python' run.py",
            instruction,
        )
        self.assertNotIn("&& python3 run.py", instruction)

    def test_codex_cli_finalizer_falls_back_to_python3_without_prepared_python(self):
        with patch.dict(os.environ, {}, clear=True):
            instruction = DEVSBackendService._codex_cli_run_instruction("demo")
    ...
    def test_finalizer_changed_source_files_ignore_internal_logs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            log_root = project_root / "devs_project" / "_analysis_logs"
            cache_root = project_root / "devs_project" / "__pycache__"
            log_root.mkdir(parents=True)
            cache_root.mkdir(parents=True)
            (project_root / "run.py").write_text("print('before')\n", encoding="utf-8")
            (log_root / "codex.txt").write_text("before\n", encoding="utf-8")
            (cache_root / "run.pyc").write_bytes(b"before")

            before = service._get_snapshot(tmp)
            (project_root / "run.py").write_text("print('after')\n", encoding="utf-8")
            (log_root / "codex.txt").write_text("after\n", encoding="utf-8")
            (cache_root / "run.pyc").write_bytes(b"after")
            after = service._get_snapshot(tmp)

            changed_files = service._detect_updated_project_files(tmp, before, after)
            self.assertIn("demo/run.py", changed_files)
            self.assertIn("demo/devs_project/_analysis_logs/codex.txt", changed_files)

            self.assertEqual(
                service._changed_source_files(changed_files),
                ["demo/run.py"],
            )
    ...
    def test_chat_history_preserves_long_prior_user_requirements(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            session_id = current_session_id(service)
            long_requirement = "Detailed requirement " + ("A" * 9_000)
            service._save_message(
                session_id,
                {
                    "message_id": "msg_long_user",
                    "session_id": session_id,
            ...
            prompt = service._build_agent_prompt(
                session_id,
                "req_current",
                "Apply this extra constraint.",
                None,
                False,
            )

            self.assertIn(long_requirement, prompt)
            self.assertNotIn("...[truncated]", prompt)

    def test_final_evaluation_links_latest_project_request_and_version(self):
        with tempfile.TemporaryDirectory() as tmp:
            write_project(tmp, "rated_project")
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            session_id = current_session_id(service)
            project = service.list_projects(session_id)[0]
            ...
            project["validation"] = {
                "status": "ready",
                "execution_id": "exec_rated_version",
                "bundle_digest": "digest-rated-version",
            }
            service._save_projects(session_id, [project])
            service._save_request(
                session_id,
                {
                    "request_id": "req_latest_project",
                    "session_id": session_id,
                    "status": "completed",
                    ...
                    "updated_project_ids": [project_id],
                    "updated_project_names": ["rated_project"],
                    "started_at": None,
            ...
            evaluation = service.submit_evaluation(
                session_id,
                stage="final",
                request_id="req_unrelated_active_request",
            ...
            self.assertEqual(evaluation["request_id"], "req_latest_project")
            self.assertEqual(evaluation["payload"]["project_name"], "rated_project")
            self.assertEqual(evaluation["payload"]["project_status"], "ready")
            self.assertEqual(evaluation["payload"]["project_version"], 7)
            self.assertEqual(
                evaluation["payload"]["validation_execution_id"],
                "exec_rated_version",
            )
            self.assertEqual(
                evaluation["payload"]["bundle_digest"],
                "digest-rated-version",
            )
            self.assertEqual(evaluation["payload"]["study_group"], "class-a")

    def test_build_success_result_is_formatted_for_chat(self):
        message = DEVSBackendService._format_assistant_result_message(
            "Build Success!\nRoot Model: demo/devs_project/Demo.py\nBuild Summary: {}",
            error=None,
            result_title="Simulation ready",
            result_detail="The exact generated runner passed its bounded test.",
            updated_project_names=["demo"],
            interface_output_count=1,
        )

        self.assertIn("Simulation ready.", message)
        self.assertIn("Project: demo", message)
        self.assertIn("Next: open the Run panel", message)
        self.assertIn("Raw build details remain available", message)
        self.assertNotIn("Root Model:", message)
    ...
    def test_generation_requirements_include_complete_intent_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)

            requirements = service._generation_requirements_from_intent(
                {
                    "requirements": "Build a deterministic shop model.",
                    "custom_arrival_profile": {
                        "rates_per_hour": [10, 110, 40, 10, 5],
                        "windows_minutes": [[0, 60], [60, 120]],
                    },
                }
            )

            self.assertIn("complete interpreted intent JSON", requirements)
            self.assertIn("custom_arrival_profile", requirements)
            self.assertIn("110", requirements)

    def test_intent_preserves_minimal_stdout_output_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            service = DEVSBackendService(DummyAgent(), tmp, start_worker=False)
            ...
            user_content = (
                ...
                "Format: JSONL.\n"
                'Each record MUST follow: `{"time": <float>, "entity": <str>, '
                '"event": <str>, "payload": <dict>}`.'
            )

            normalized = service._normalize_intent_payload({}, user_content)
            ...
            requirements = service._generation_requirements_from_intent(normalized)
            self.assertIn('"output_contract"', requirements)
            self.assertIn('\\"payload\\": <dict>', requirements)

            generic = service._normalize_intent_payload(
                {},
                "Build a restaurant queue simulation.",
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def interpret_intent(**_kwargs):
                return {
                    ...
                    "entities": ["Factory", "Retailer"],
                    "event_flow": ["Factory ships inventory to retailer."],
                    "parameters": ["Demand rate defaults to 12 per day."],
            ...
            self.assertIn("Factory supplies a retailer.", prepared_requirements[-1])
            self.assertIn("Daily demand is stochastic.", prepared_requirements[-1])
            self.assertIn("Factory ships inventory to retailer.", prepared_requirements[-1])
            self.assertIn("Demand rate defaults to 12 per day.", prepared_requirements[-1])
            self.assertIn("Stockout count", prepared_requirements[-1])
