...
if __name__ == "__main__":
    ...
    first_event_time = float(sim.time_next)
    if math.isfinite(first_event_time) and first_event_time <= numeric_horizon:
        # xDEVS 3 interprets simulate_time() as an interval beginning at the
        # first pending event, not as an absolute clock value.
        sim.simulate_time(
            max(0.0, numeric_horizon - first_event_time) + 1e-9
        )
    sim.exit()
