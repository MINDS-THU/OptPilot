...
```json
{
  ...
  "catalog_counts": {"environments": 4, "methods": 9, "studies": 12},
  "run_count": 6,
...
| Tool | Purpose |
| --- | --- |
| `optpilot_catalog_list` | List environments, methods, and studies |
| `optpilot_catalog_detail` | Inspect a catalog config and contract |
...
```json
{
  ...
  "targets": ["user_catalog/studies/job_shop_ortools_cpsat.yaml"],
...
- What OpenHands API should be considered stable enough for the first bridge:
  SDK, REST agent server, or a local subprocess wrapper?
