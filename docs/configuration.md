...
`config` selects the schema. The older internal `kind: StudySpec` format is generated by OptPilot and written to run directories; users normally do not write it.
...
| Python import | `package.module:function` or `package.module:Class`. Public configs do not use the internal `python:` prefix. |
...
evaluator:
  ...
  # evaluate(artifact_spec, instance, context) -> dict
...
runtime:
  sandbox: host          # enum: host | container | external
...
# Optional files to save from each trial workspace.
artifacts:
  - metrics.json
  - path: logs/*.txt
    name: logs
...
For parameter candidates, `artifact_spec` is the candidate parameter dictionary. For file candidates, it contains the trial workspace, candidate root, manifest path, and candidate file records.
...
`files` candidates are file artifacts. `trialWorkspace` seeds the workspace; the method returns references to generated files; the materializer copies those files into `candidate.materialize.root`.
...
`opaque` candidates are for custom method/environment pairs that share their own artifact semantics:

```yaml
candidate:
  format: opaque
  opaque:
    family: my-custom-artifact
```
...
execution:
  backend: local         # enum: local | local_subprocess | external
  parallelism: 2
  timeoutSeconds: 600

evidence:
  level: standard        # enum: minimal | standard | full
  artifactStorage: reference # enum: reference | copy
...
`execution.backend: external` is accepted by the public schema for future remote/custom backends, but it is not implemented by the current runner.
