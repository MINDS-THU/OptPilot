...
sequenceDiagram
  ...
  loop "until budget stops"
    ...
    Method-->>Runner: "candidate artifacts"
    ...
    Evaluator-->>Runner: "status, metrics, artifacts, records"
    Runner->>Store: "write trials, observations, artifacts, events"
...
- `evidence`: previous observations, artifacts, trials, records, method calls, and events.
...
`optpilot.code_artifacts.CodeArtifactStore` creates this file-candidate shape for Python methods.
...
- The candidate `spec` is passed directly as the runtime artifact spec.
...
## Candidate Store And Evidence Artifacts
...
| `run_dir/evidence_artifacts/` | Optional retained copies of evaluator output files when `evidence.artifactStorage: copy` is enabled. |

The trial workspace is what gets evaluated. The candidate store is the handoff area used before materialization. The evidence artifact area stores copies of output files after evaluation when the study requests copied evidence instead of path references.
...
The evaluator receives the materialized candidate and one instance. It returns or writes status, metrics, optional artifacts, and optional record streams. The configured adapter normalizes those into observations.
...
```yaml
execution:
  backend: local          # local | local_subprocess | external
  parallelism: 2
  runtime:
    sandbox: host         # host | container | external
```
...
| `backend: external` | Public schema exists; runner support is not implemented yet. |
| `sandbox: external` | Public schema exists; runner support is not implemented yet. |
...
| `artifacts.jsonl` | Candidate validation and materialization records. |
