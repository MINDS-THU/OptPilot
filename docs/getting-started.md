...
!!! note "Every `optpilot run` needs its own `--package-root`"
    ...
    COOPA, and tutorial commands below each use their own package root.
...
is the contract used by OptPilot's code-generating research methods.
...
`catalog/or_solving`, or `catalog/optpilot_tutorial`), not `catalog/` itself.
To confirm a package is
...
The public catalog has three research packages and one tutorial package. Each
has its own package root and a clear role:

| Goal | Package root | Needs |
| --- | --- | --- |
| Learn how a package fits together | `catalog/optpilot_tutorial` | Nothing extra |
| Generate or evaluate a DEVS simulator | `catalog/devs_gallery` | Examples: nothing; generation: `OPENROUTER_API_KEY` |
| Improve a simulation policy from traces | `catalog/production_agv_scheduling` | Baselines: nothing; language-model search: `OPENROUTER_API_KEY` |
| Solve one OR problem from text | `catalog/or_solving` | `OPENROUTER_API_KEY` and the pruned runtime deps (COOPA itself is bundled) |

### Learn with the tutorial package

Start with `catalog/optpilot_tutorial` when you want the smallest complete
example of a package, Environment, Method, Resource, and Run setup:
...
See [Build Your First Package](tutorial-package.md) for the guided tour and a
copyable structure for your own package.
...
1. Walk through `catalog/optpilot_tutorial/` to learn the package structure,
   then browse `catalog/production_agv_scheduling/` — the most complete package,
...
[LLM-Guided Heuristic Design](llm-policy-search.md).
